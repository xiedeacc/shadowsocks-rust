...
- `proxy_local_output`：是否代理运行 sslocal 的机器本机发起的非 DNS TCP/UDP OUTPUT 流量。默认 `false`。
  ...
  Shadowsocks server 端点，避免内网通信受影响或代理出口回环。开启时进程还会给 sslocal 自身的出站
  socket 打上一个专用 fwmark（默认 `0x5356`，或复用已配置的 `outbound_fwmark`），并在 OUTPUT
  重定向/ tproxy 链顶部加入 `meta mark <mark> return`，以**身份**而非 server IP 豁免代理出口，
  从而即使 server 用域名且 IP 轮换也不会回环。
- `global_proxy`：全局透明代理开关。默认 `false`。开启后，除固定内网/保留网段与 `direct4/direct6`
  set 外的所有目的地址都走代理：nft 规则从「命中 `@proxy4` 才重定向」翻转为「`daddr != @direct4`
  即重定向」。同时 `route_ip_inner` / `route_domain_inner` 在 `global_proxy` 开启时短路返回 Proxy
  （固定 Direct IP 仍返回 Direct），学习式 temporary/persistent 优先级在全局模式下被绕过。
- `client_global_proxy_ips`：按源 IP 强制走代理的列表。默认空。即使 `global_proxy=false`，来自这些
  源 IP 的非 53 流量也会被 `ip saddr @client_proxy4 ... redirect` 强制走代理（该规则仅在
  `global_proxy=false` 时生成——全局模式下本就全部代理）。
- `client_direct_ips`：按源 IP 强制直连的列表。默认空。即使 `global_proxy=true`，来自这些源 IP 的
  非 53 流量也通过 `ip saddr @client_direct4 ... dport != 53 return` 直连；其 DNS（53 端口）仍进入
  本地 DNS 拦截，并强制走**国内**解析器（`route_domain_for_source` 对这些源返回 Direct），以复用
  cache 并避免拿到无法直连的国外 IP。SOCKS/HTTP 显式代理入口也会尊重该列表（Futu 学习实例除外）。
- `dns_ipv4_only`：是否过滤 AAAA 响应。默认 `true`。开启时本地 DNS 短路 AAAA 查询，且**不安装**
  IPv6 的重定向/ tproxy 规则，避免把 LAN IPv6 流量导向只监听 IPv4 的 redir 端口而被黑洞。
...
- Linux 启动时会尽力删除遗留的 `inet ssrust_redir` 表。
...
Linux/OpenWrt 防火墙拦截使用 `inet ssrust_redir` 表。当前表中会创建八个 interval（auto-merge）set：

- `direct4` / `direct6`：直连 IP（含固定内网/保留网段与 Direct 规则）。
- `proxy4` / `proxy6`：代理 IP（DNS 学习到的、`proxy_ip` 文件中的、Futu SOCKS 学习的）。
- `client_proxy4` / `client_proxy6`：`client_global_proxy_ips` 中的源 IP（按源强制代理）。
- `client_direct4` / `client_direct6`：`client_direct_ips` 中的源 IP（按源强制直连）。

透明代理的目的地匹配取决于 `global_proxy`：

- **关闭（学习模式）**：命中 `daddr @proxy4 / @proxy6` 才重定向/ tproxy 到 redir 端口；同时为
  `@client_proxy4/6` 源生成 `saddr ... redirect` 规则。
- **开启（全局模式）**：`daddr != @direct4 / != @direct6` 即重定向——此模式下 `direct4` / `direct6`
  被规则直接依赖（不再是「仅辅助」）。

两种模式下 `@client_direct4/6` 源的非 53 流量都在链顶 `... dport != 53 return` 直连。TCP 使用 NAT
`redirect`，UDP 非 53 使用 tproxy，均指向同一 redir 端口。`dns_ipv4_only` 开启时不安装 IPv6 的代理
规则（见配置说明）。
...
flowchart TD
    ...
    B --> C[删除旧 inet ssrust_redir 表]
    C --> D[创建 inet ssrust_redir 表]
...
`DnsInterceptGuard` 在 drop 时会删除 `inet ssrust_redir` 表并清理 tproxy 策略路由（`ip rule` + `table 100`）。
这只在**优雅退出**（SIGTERM/SIGINT，由信号 monitor 触发）时执行。由于 release 采用 `panic = "abort"`，
panic 不会展开、drop 不会运行，因此进程在安装防火墙时会注册一个 **panic hook**：在 abort 之前执行同样的
删表 + tproxy 清理，避免把 DNS/重定向规则留给一个已不存在的监听端口（否则全 LAN DNS 会被黑洞）。

对于 SIGKILL / 断电 / procd 放弃重启（`respawn` 预算耗尽）等进程无法捕获的情况，由以下机制兜底：

- 进程**每次启动**都会无条件清理遗留的 `inet ssrust_redir` 表与 tproxy 策略路由（不再只在
  非 firewall 模式下清理）；`setup_nft` 本身也是「先 `delete table` 再重建」，因此 procd 正常重启即自愈。
  防火墙规则在 DNS 监听端口**绑定之后**才安装，避免启动瞬间的黑洞窗口。
- OpenWrt init 脚本 `/etc/init.d/shadowsocks` 在 `start_service` / `restart_service` / `reload_service`
  以及 `stop_service()` 中调用 `cleanup_firewall`（删表 + 删 tproxy `ip rule`/route），因此普通 `stop`
  也能恢复纯净状态；web admin 触发的代码重启也会先执行同样清理。

无论走哪条路径，LAN/路由器管理面（SSH、网关 `192.168.2.1`）都不会被切断——固定内网/保留网段始终位于
每条重定向/ tproxy 链的**最顶部** `return`，因此最坏情况只是代理/ DNS 短时中断而非「砖机」。

运维清理命令：`deploy/scripts/deploy_openwrt.sh --cleanup` 会 SSH 到路由器停服务并清理 `inet ssrust_redir`
表 + tproxy `ip rule`/route（幂等）。conntrack 刷新依赖路由器安装 `conntrack-tools`，
缺失时仅跳过并告警一次。
...
iptables 回退不维护 Proxy IP set，因此不能提供 nft Proxy set 驱动的非 53 TCP redirect 或 UDP tproxy 能力。
...
flowchart TD
    ...
    G -- 是 --> H[nft list set inet ssrust_redir proxy4/proxy6]
    H --> I[解析 nft 输出并检查匹配]
    G -- 否 --> J[跳过 nft 检查]
    I --> K[返回文件匹配和 nft 匹配]
    J --> K
...
- Linux nftables 透明代理规则同时支持 `redirect` 和 `tproxy` 两种机制：
  ...
  - `tproxy` 属于透明代理路径，配合 mark、policy routing 和 `IP_TRANSPARENT`；当前 Linux firewall 自动安装的 UDP Proxy IP 规则使用 `tproxy ip/ip6 to :redir_port`，并排除 dport 53，避免影响 DNS 拦截。
