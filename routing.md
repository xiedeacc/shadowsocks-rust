...
- Temporary Lists 不会写入 `direct_ip.txt`、`direct_domain.txt`、`bypass_ip.txt` 或 `bypass_domain.txt`。
- `bypass_domain.txt` 通常由 `data/source/gfw.txt` 生成；`bypass_ip.txt` 主要保存运行时从 DNS 学到的 proxy IP。
...
flowchart TD
    ...
    G -- 否 --> I{temporary.bypass_domain 匹配?}
    I -- 是 --> J[decision=Proxy]
    I -- 否 --> K{persistent.direct_domain 匹配?}
    K -- 是 --> H
    K -- 否 --> L{persistent.bypass_domain 匹配?}
    L -- 是 --> J
    ...
    AA[连接路径收到目标地址] --> AB{目标是域名还是 IP?}
    AB -- 域名 --> F
    AB -- IP --> AC{temporary.direct_ip 匹配?}
    AC -- 是 --> AD[连接 Direct]
    AC -- 否 --> AE{temporary.bypass_ip 匹配?}
    AE -- 是 --> AF[连接 Proxy]
    AE -- 否 --> AG{persistent.direct_ip 匹配?}
    AG -- 是 --> AD
    AG -- 否 --> AH{persistent.bypass_ip 匹配?}
    AH -- 是 --> AF
    AH -- 否 --> AI[未命中 routing rule, 继续 ACL/reverse cache]
    H --> AD
    J --> AF
...
- 记录 DNS 响应并调用 `add_dns_results`，让 Proxy DNS 结果学习到 `bypass_ip.txt` 和 nft bypass set。
