...
当前透明代理规则实际使用的是 `proxy4` / `proxy6`：TCP 使用 NAT `redirect` 到 redir 端口，UDP 非 53 使用 tproxy 到同一个 redir 端口。Direct 优先级主要通过过滤或删除 Proxy set 中与 Direct 重叠的元素实现。`direct4` / `direct6` 作为辅助 set 创建，但当前透明代理规则不依赖它们。
