...
Linux/OpenWrt 防火墙拦截使用 `inet ssrust_dns` 表。当前表中会创建八个 interval（auto-merge）set：
...
`DnsInterceptGuard` 在 drop 时会删除 `inet ssrust_dns` 表并清理 tproxy 策略路由（`ip rule` + `table 100`）。
...
- 进程**每次启动**都会无条件清理遗留的 `inet ssrust_dns` 表与残留的 iptables DNS 重定向（不再只在
  非 firewall 模式下清理）；`setup_nft` 本身也是「先 `delete table` 再重建」，因此 procd 正常重启即自愈。
  防火墙规则在 DNS 监听端口**绑定之后**才安装，避免启动瞬间的黑洞窗口。
- OpenWrt init 脚本 `/etc/init.d/shadowsocks-rust` 在 `start_service` / `restart_service` / `reload_service`
  以及新增的 `stop_service()` 中调用 `cleanup_firewall`（删表 + 删 tproxy `ip rule`/route + 清 iptables
  重定向），因此普通 `stop` 也能恢复纯净状态。
- 一个独立的 procd **看门狗实例** `ssrust-watchdog.sh` 周期检测：若 sslocal 进程不在但 `ssrust_dns`
  表仍在（崩溃循环导致 procd 放弃重启、或被 SIGKILL），则删表恢复到「无 sslocal」的纯净状态。

无论走哪条路径，LAN/路由器管理面（SSH、网关 `192.168.2.1`）都不会被切断——固定内网/保留网段始终位于
每条重定向/ tproxy 链的**最顶部** `return`，因此最坏情况只是代理/ DNS 短时中断而非「砖机」。

运维清理命令：`deploy/scripts/deploy_openwrt.sh --cleanup` 会 SSH 到路由器停服务并清理 `inet ssrust_dns`
表 + tproxy `ip rule`/route + iptables 重定向（幂等）。conntrack 刷新依赖路由器安装 `conntrack-tools`，
缺失时仅跳过并告警一次。
